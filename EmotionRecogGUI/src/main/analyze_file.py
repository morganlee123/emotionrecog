...
for filepath in glob.glob("seg*"):
    os.remove(filepath)

#os.system("sox " + WAVE_OUTPUT_FILENAME + " seg.wav trim 0 " + str(SEGMENT_LENGTH) + " : newfile : restart")
from pydub import AudioSegment
#import pydub
#pydub.AudioSegment.ffmpeg = "./ffmpeg"
AudioSegment.converter = '/Users/morgan/Work/MSU/IPROBE/research/EmotionRecogGUIDev/EmotionRecogGUI/src/main/ffmpeg'
# Segment the new audio
t1 = 0 * 1000 #Works in milliseconds
t2 = SEGMENT_LENGTH * 1000
original_audio = AudioSegment.from_wav(WAVE_OUTPUT_FILENAME)
length = original_audio.duration_seconds
needed_segs = int(length) // SEGMENT_LENGTH
for i in range(needed_segs):
    newAudio = original_audio[t1:t2]
    newAudio.export('seg'+str(i)+'.wav', format="wav") #Exports to a wav file in the current path.
    t1 = t2
    t2 = t2+SEGMENT_LENGTH*1000


time_domain = 0
for file in glob.glob("seg*"):
    ...
    y, sr = librosa.core.load(file, sr=sample_rate, offset=0.5)
    ...
    print(str(time_domain) + ","+ pred_string)
